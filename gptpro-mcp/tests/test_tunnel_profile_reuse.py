...
BASE_CLI = SKILL_ROOT.parent / "gptpro" / "scripts" / "gptpro.py"
...
class TunnelProfileReuseTests(unittest.TestCase):
    ...
    def run_cli(self, *args: str, expected: int = 0) -> subprocess.CompletedProcess[str]:
        result = subprocess.run(
            [sys.executable, str(SCRIPT), "--base-entrypoint", str(BASE_CLI.resolve()), *args],
            text=True,
            capture_output=True,
            env=self.env,
            check=False,
    ...
    def test_machine_global_standing_approval_reuses_scope_and_revocation_blocks_use(self) -> None:
        ...
        preview = json.loads(self.run_cli(*common, "--dry-run").stdout)
        self.assertTrue(preview["dry_run"])
        self.assertEqual("all-local-git", preview["profile"]["scope"]["repository_scope"])
        self.assertEqual("desktop-ui", preview["profile"]["scope"]["delivery_channel"])
        self.assertEqual(["**"], preview["profile"]["scope"]["path_patterns"])
        self.assertFalse(state_root.exists())
        self.assertFalse((self.repo / ".gptpro" / "standing-approvals").exists())

        created = json.loads(
            self.run_cli(*common, "--confirm-standing-approval").stdout
        )
        profile_path = state_root / "standing-approvals" / f"{standing_name}.json"
        self.assertTrue(profile_path.is_file())
        self.assertEqual(0o600, stat.S_IMODE(profile_path.stat().st_mode))
        self.assertEqual(
            "gptpro-standing-approval-v2", created["profile"]["contract"]
        )

        listed = json.loads(
            self.run_cli(
                "standing-approval-list", "--state-root", str(state_root)
            ).stdout
        )
        ...
        self.assertEqual("gptpro-standing-approval-v2", approved["approval_source"])
        self.assertEqual("approved", self.load(in_scope / "state.json")["phase"])
        ...
        self.assertIn("DESKTOP_APPROVAL_REVOKED", rejected.stderr)
