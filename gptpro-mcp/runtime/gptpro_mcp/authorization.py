...
class AuthorizationGrant:
    ...
    def validate(self, requested_package_id: str, *, now: datetime | None = None) -> None:
        ...
        expected_channel = "desktop-ui" if schema_version == 4 else "browser"
        if delivery != {"channel": expected_channel, "approval_required": True}:
            raise ToolError(
                "CHANNEL_NOT_APPROVED",
                "The package delivery channel is not approved for this schema.",
                recovery=f"Prepare and approve a package for the {expected_channel} delivery channel.",
            )
