...
- PR 리뷰 재현에서 발견된 실험 간 세션 재사용, arm별 정책 버전 혼합, 리더 설정 불일치, 제외된 차단 시도의 집계 누락을 수정했습니다. 보고서는 비교된 시도와 arm별 전체 시도를 나누며 입력은 schema 1, 출력은 schema 2입니다.
- standalone 수정 파일과 Plugin 미러를 동일하게 반영하고 기존 버전 기대값을 갱신했습니다. marketplace inventory, 다른 Plugin·인증·설치 환경은 변경하지 않습니다.
