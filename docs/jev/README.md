...
| `jev-decision` | 명시한 프로젝트 기준으로 좁은 구현·작업 후보 선택 |
...
| `jev-product-choice` | 명시한 사용자 기준으로 좁은 문구·UX 후보 선택 |
...
판단 스킬은 후보·기준·검증 방법이 분명한 텍스트 작업에 우선 적용합니다. 넓은 프로젝트 가치 선택에는 [프로젝트 적합성 안내](../../jev-workbench/references/project-fit.md)에 따라 필수 제약과 사용자 선호의 우선순위를 확인합니다. Jev 1.13의 한국어 정확도, 긴 상태 입력, 다단계 추론 한계는 실제 프로젝트 사례로 평가해야 합니다.
...
```text
$jev-decision 프로젝트 제약과 선호 기준을 확인하고, 위임한 좁은 후보 중에서만 Jev로 선택해주세요.
