...
class RoutingEvaluationTests(unittest.TestCase):
    ...
    def test_unknown_metrics_stay_null(self):
        report = evaluator.summarize(fixture())
        self.assertIsNone(report["arms"]["single-astra"]["metrics"]["credits"]["total"])
        self.assertIsNone(report["arms"]["single-astra"]["metrics"]["leader_rework_minutes"]["mean"])
    ...
    def test_failed_and_incomplete_attempts_are_not_hidden(self):
        ...
        report = evaluator.summarize(data)
        self.assertEqual(report["matched_batches"], 1)
        self.assertEqual(report["arms"]["astra-only"]["pass_fraction_of_attempts"], 0)
        self.assertEqual(report["arms"]["mixed-model"]["verification_counts"]["BLOCKED"], 1)
