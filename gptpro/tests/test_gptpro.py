...
class ControllerCase(PackageCase):
    ...
    def test_thinking_effort_is_forwarded_to_the_exact_runtime_model(self) -> None:
        ...
        fake = FakeProcess([
            {"type": "submitted", "request_id": "request-1"},
            {"type": "complete", "text": "완료", "conversation_id": "c", "parent_message_id": "m", "tool_routes": 0, "completion_source": "direct-desktop-stream-v1", "stream_handoff_topic_sha256": None},
        ])
    ...
    def test_marker_collision_preserves_raw_and_blocks_resend(self) -> None:
        ...
        fake = FakeProcess(
            [
                {"type": "submitted", "request_id": "request-1"},
                {"type": "complete", "text": f"collision {marker}", "conversation_id": "c", "parent_message_id": "m", "tool_routes": 0, "completion_source": "direct-desktop-stream-v1", "stream_handoff_topic_sha256": None},
            ]
        )
        with self.assertRaises(controller.ControllerError) as raised:
            self.run_fake(handoff, fake)
    ...
    def test_nonzero_tool_route_is_never_imported(self) -> None:
        ...
        fake = FakeProcess(
            [
                {"type": "submitted", "request_id": "request-1"},
                {"type": "complete", "text": "tool answer", "tool_routes": 1, "completion_source": "direct-desktop-stream-v1", "stream_handoff_topic_sha256": None},
            ]
        )
        with self.assertRaises(controller.ControllerError) as raised:
            self.run_fake(handoff, fake)
