...
**패키지는 생성한 gptpro 버전에서만 사용합니다.** 현재 버전 `0.6.1`은 manifest에 `runtime_version`을 기록하고, 값이 없거나 현재 버전과 다르면 `PACKAGE_VERSION_UNSUPPORTED`로 거부합니다. 업데이트 후 구버전 패키지의 검증·승인·상담·응답 복구를 지원하지 않으며, 이전 승인도 그 패키지를 다시 사용할 권한이 되지 않습니다. 기존 파일은 그대로 두고 자동 변환·삭제하지 않습니다. 새 상담은 현재 버전에서 새 패키지로 준비하며, 이미 전송했거나 전송 여부가 불명확한 요청을 자동으로 다시 보내지 않습니다.
...
Git 삭제는 현재 filesystem에서의 부재와 구분합니다. `git rm --cached`로 남긴 파일은 `--allow-untracked`일 때만 현재 파일 블록에 `tracked: false`로 포함되며, 같은 경로의 HEAD 삭제 diff도 별도로 유지합니다. 이 경우 tracked-only 지속 승인으로는 현재 untracked 내용을 승인할 수 없습니다. 현재 파일과 삭제 경로가 겹쳐도 파일 한도와 승인 경로 목록에서는 한 경로로 셉니다. 파일↔디렉터리 교체 시 현재 파일과 과거 삭제 경로를 각각 선택할 수 있으며, 과거 경로 하나를 선택했다고 비선택 하위 파일까지 diff에 포함하지 않습니다.
