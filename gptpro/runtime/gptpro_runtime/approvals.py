...
def create_standing(
    ...
    *,
    ...
    path_rules = {
        ...
        "exact_paths": sorted([item["path"] for item in manifest["files"]] + manifest["diff"].get("deleted_paths", [])) if selection.get("file_list_sha256") else [],
...
def standing_matches(approval: dict[str, Any], manifest: dict[str, Any]) -> tuple[bool, str]:
    ...
    manifest_paths = [item["path"] for item in manifest["files"]] + manifest["diff"].get("deleted_paths", [])
    paths_allowed = all(
