...
The runtime intentionally remains a legacy MCP server. Current clients can first send `server/discover` for the 2026-07-28 stateless protocol and then fall back to the legacy `initialize` handshake when the method is not supported. Because Tunnel connector commands share one persistent stdio child, compatibility traffic can contain an earlier completed legacy lifecycle before that discovery fallback. Receiving `server/discover` closes tool readiness while returning `-32601`, permits exactly one fresh legacy `initialize`, and keeps tools denied until a new `notifications/initialized`. An ordinary duplicate `initialize` remains an error, and an earlier lifecycle cannot authorize subsequent ChatGPT tool calls by itself.

Each authorized stdio child appends one package-local diagnostic trace. The mode-`0600` hash chain binds the package/session/manifest/approval, immutable archive/file set/tool schema, disclosure-audit header, Tunnel profile/client, exact MCP target, and runtime-tree hashes. At most 64 events are retained; the final slot is reserved for an explicit `trace_truncated` marker when more traffic occurs. Events contain only sequence, fixed method/stage/outcome/readiness enums, requested-version classification, exact requested version only from the fixed supported-version allowlist, and allowlisted negotiated version. `decision`, post-mutation notification `processed`, and local `response_flushed` stages distinguish a decision from actual state application and successful local stdio flush.
