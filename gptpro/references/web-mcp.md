...
The logged-in ChatGPT tool-execution path has also been observed to omit `notifications/initialized` while sending `initialize`, one identical supported-version replay, and then a `tools/call` over the same Tunnel child. A narrowly detected request-scoped compatibility path accepts only a structurally valid call to one of the three allowlisted tools after that exact sequence and only when no `server/discover` was seen. That call records an explicit sanitized readiness transition; while this mode is active, later identical-version `initialize` requests are idempotent. A missing replay, different version, malformed or unknown tool call, `tools/list`, or any discovery-path call remains denied until the standard notification. This compatibility transition is not authorization: every call still revalidates the exact active package, immutable hashes, expiry, budgets, and disclosure audit before returning content.
