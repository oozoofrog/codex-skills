...
Before any future write-tool design review, an operator may run a separate approved E2E with `mcp-activate --diagnose-request-correlation`. The diagnostic reads the official v0.0.12 private in-memory admin log over the activation-owned Unix socket after authorization revoke, immediately replaces allowlisted outer request IDs with per-session HMACs, and aligns them by order with a complete receipt-bound protocol trace and disclosure audit. It can distinguish repeated copies of one outer request from distinct outer requests only when every window and count matches; otherwise it reports `inconclusive`. It never stores raw IDs/payloads, never changes budgets, and never deduplicates. See [Request-correlation diagnostic](request-correlation.md).
