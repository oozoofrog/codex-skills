...
class McpResearchTests(unittest.TestCase):
    ...
    def mark_submitted(self, handoff: Path, store: object) -> None:
        verified = self.module.verify_package(handoff)
        plan = self.module.build_handoff_plan(handoff_dir=handoff, verified=verified)
        with (
    ...
    def test_supplement_flag_is_rejected_in_desktop_only_schema4(self) -> None:
        rejected = self.run_cli(
            *self.prepare_arguments(supplement=self.supplement),
            expected=2,
        )
        self.assertIn("DESKTOP_EXTERNAL_DOCUMENT_REQUIRES_EVIDENCE", rejected.stderr)
        self.assertEqual(
