"""Private, one-active-package runtime state for the read-only MCP lifecycle."""
...
def observe_archived_runtime_state(
    root: Path, session_id_sha256: str
) -> dict[str, Any] | None:
    """Read one exact archived terminal session without creating or locking state."""
    ...
    try:
        sessions = _directory_fd(requested / "sessions")
    except RuntimeStateError as exc:
        try:
            (requested / "sessions").lstat()
        except FileNotFoundError:
            return None
        raise exc
    descriptor = -1
    try:
        metadata = os.fstat(sessions)
        if (
            ...
            raise RuntimeStateError(
                "RUNTIME_STATE_UNSAFE",
                "The archived runtime directory must be owner-only mode 0700.",
            )
        try:
            descriptor = os.open(
            ...
            raise RuntimeStateError(
                ...
                "The archived runtime state cannot be inspected.",
            ) from exc
        _validate_private_fd(descriptor)
        value = RuntimeStateStore._read_state_descriptor(descriptor)
        if (
