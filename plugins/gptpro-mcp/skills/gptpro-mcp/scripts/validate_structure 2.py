#!/usr/bin/env python3
"""Validate the Desktop read-only gptpro-mcp Skill structure."""
...
REQUIRED_FILES = (
    ...
    "runtime/__init__.py",
    "runtime/gptpro_desktop/__init__.py",
    "runtime/gptpro_desktop/approval.py",
    "runtime/gptpro_desktop/binding.py",
    "runtime/gptpro_desktop/contract.py",
    "runtime/gptpro_desktop/state.py",
    ...
    "runtime/gptpro_mcp/component_compat.py",
    ...
    "runtime/gptpro_mcp/residual_ownership.py",
    ...
    "tests/test_desktop_workflow.py",
    ...
    "tests/test_tunnel_profile_reuse.py",
)
...
def validate_frontmatter(skill_root: Path, errors: list[str]) -> None:
    ...
    expected_invocation = f"${name}"
    if expected_invocation not in (skill_root / "agents/openai.yaml").read_text(encoding="utf-8"):
        errors.append(
            f"agents/openai.yaml default prompt must explicitly invoke {expected_invocation}"
        )
...
def validate_python(skill_root: Path, errors: list[str]) -> None:
    python_files = {
        skill_root / "scripts/component_handshake.py",
...
def validate_component_boundary(skill_root: Path, errors: list[str]) -> None:
    forbidden_paths = (
        "runtime/gptpro_browser",
        "references/browser-first.md",
        "references/browser-handoff.md",
        "references/browser-policy.md",
        "references/response-monitor.md",
        "tests/test_browser_first.py",
    )
    for relative in forbidden_paths:
        if (skill_root / relative).exists():
            errors.append(f"Desktop-only component retains removed path: {relative}")
    try:
        cli = (skill_root / "scripts/gptpro.py").read_text(encoding="utf-8")
        handshake = (skill_root / "scripts/component_handshake.py").read_text(encoding="utf-8")
        component_compat = (
            skill_root / "runtime/gptpro_mcp/component_compat.py"
        ).read_text(encoding="utf-8")
        compatibility = (skill_root / "references/component-compatibility.md").read_text(
            encoding="utf-8"
        )
    except (OSError, UnicodeDecodeError) as exc:
        errors.append(f"Unable to inspect component compatibility boundary: {exc}")
        return
    required_cli = {
        ...
        '"gptpro-component-capabilities-v1"',
    }
    required_handshake = {
        "GPTPRO_BASE_COMPONENT_REQUIRED",
        "GPTPRO_BASE_INCOMPATIBLE",
        "gptpro-install-descriptor-v1",
        "--base-entrypoint",
        "capabilities",
    }
    required_docs = {
        ...
        ".gptpro-components.json",
        "fail-closed",
    }
    for label, text, required in (
        ("MCP capabilities", cli, required_cli),
        ("component handshake", handshake + component_compat, required_handshake),
        ("compatibility matrix", compatibility, required_docs),
    ):
        missing = sorted(token for token in required if token not in text)
        if missing:
            errors.append(f"{label} is missing required contract terms: {missing}")
    forbidden_cli = {
        "command_browser_plan",
        "command_response_monitor_plan",
        "command_record_response_monitor_started",
        "command_record_response_monitor_stopped",
        "runtime.gptpro_browser",
        "remote-debugging-port",
        "electronBridge",
    }
    retained = sorted(token for token in forbidden_cli if token in cli)
    if retained:
        errors.append(f"Desktop-only CLI retains removed delivery code: {retained}")
    forbidden_discovery = {"shutil.which(", "glob.glob(", "Path.home()", "os.walk("}
    found = sorted(
        token for token in forbidden_discovery if token in handshake + component_compat
    )
    if found:
        errors.append(f"Component handshake contains forbidden path discovery: {found}")
...
def validate_mcp_foundation(skill_root: Path, errors: list[str]) -> None:
    ...
    try:
        ...
        errors.append(f"Unable to hash read-only MCP schema fixture: {exc}")
        return
    if schema_sha256 != EXPECTED_MCP_SCHEMA_SHA256:
        errors.append(
            "Read-only MCP schema fixture does not match the trusted canonical source; "
    ...
    try:
        ...
        errors.append(f"Unable to inspect read-only MCP schema fixture: {exc}")
        return
    ...
    if unexpected:
        errors.append(f"Read-only MCP schema fixture imports non-stdlib modules: {unexpected}")
        return
    ...
    if top_level_errors or unexpected_assignments or missing_assignments:
        errors.append(
            "Read-only MCP schema fixture has unsafe or unexpected top-level structure; "
    ...
    for name, value in assignments.items():
        ...
        if any(isinstance(node, ast.Call) for node in ast.walk(value)):
            errors.append(f"Read-only MCP schema fixture assignment {name!r} contains a call")
            return
    ...
    try:
        ...
        if ast.literal_eval(assignments["MAX_TOOL_NAME_BYTES"]) != 128:
            errors.append("Read-only MCP tool-name byte bound is unsafe")
        annotations = ast.literal_eval(assignments["COMMON_ANNOTATIONS"])
        if annotations != expected_annotations:
            errors.append("Read-only MCP common tool annotations are unsafe")
            ...
            for index, entry in enumerate(catalog.elts):
                ...
                if inherited_legacy_info:
                    ...
                    if not isinstance(name_node, ast.Constant) or not isinstance(name_node.value, str):
                        raise ValidationError("Each read-only MCP tool must have one static name")
                    ...
                    if not (
                        ...
                        raise ValidationError(f"Read-only MCP tool {tool_name!r} has unsafe annotations")
        ...
        for names_assignment, catalog_assignment in (
            ...
            if ast.dump(assignments[names_assignment], include_attributes=False) != ast.dump(
                ...
                errors.append(
                    f"Read-only MCP {names_assignment} must derive only from {catalog_assignment}"
                )
    except (TypeError, ValueError, ValidationError) as exc:
        errors.append(f"Read-only MCP schema fixture validation failed: {exc}")
...
def validate_mcp_runtime_dependencies(skill_root: Path, errors: list[str]) -> None:
    ...
    paths = [
        ...
        *sorted((skill_root / "runtime/gptpro_desktop").glob("*.py")),
    ]
...
def validate_supplement_documentation(skill_root: Path, errors: list[str]) -> None:
    """Keep the Desktop external-evidence boundary explicit and reviewable."""
...
def validate(skill_root: Path, mirror: Path | None) -> dict[str, object]:
    ...
    if not root.is_dir():
        ...
        if not errors:
            ...
            validate_component_boundary(root, errors)
    ...
    return {
        ...
        "checks": [
            ...
            "read-only-mcp-bounded-schema",
            "read-only-mcp-stdlib-runtime",
            "read-only-mcp-canonical-runtime-slot",
            "desktop-mcp-research-documentation",
