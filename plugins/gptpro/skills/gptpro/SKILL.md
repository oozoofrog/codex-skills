...
- Never paste, attach, activate disclosure, or submit without valid disclosure authority. The default authority is the user's exact-package approval of outbound text, transport, delivery channel, maximum file/hash disclosure and the Schema 4 ledger policy. A repository-local bounded standing approval may satisfy that authority only when [references/standing-approval.md](references/standing-approval.md) matches the verified Schema 4 package in full; every Codex note still needs its own exact-byte approval.
...
2. Choose the handoff at the Skill layer, then prepare it with `scripts/gptpro.py prepare`. An explicit `$gptpro` project/repository request that requires Pro to inspect, search, or navigate actual project context defaults to a proposed `mcp-research` package with the smallest directed `--include` or `--file-list` scope. Before Web MCP preparation, run secretless `preflight` and pass its exact `--tunnel-profile` and confirmed profile hash into `prepare`; an existing verified profile is reusable after a Codex restart without re-reading a Tunnel ID reference. If several profiles are ready or the saved default is stale, stop for exact profile selection or repair. A new package is only a proposal until it receives either exact-package approval or a successful bounded standing-approval match; never assume a generic `$gptpro` request is authority. Use ordinary GitHub/text delivery when the user requests it or a bounded question needs only a small fixed excerpt. The CLI's default `--transport auto` remains GitHub-first and never selects Web MCP: it uses a verified, pushed GitHub commit when all selected bytes match HEAD, then falls back with an explicit warning to direct paste for a small payload or one Markdown attachment for a larger payload. Use `--transport github` to require GitHub without fallback, or `paste|text-file` to avoid repository-app access. When an explicitly selected external UTF-8 document is required without browser upload, use repeatable `--supplement LABEL=/ABSOLUTE/PATH` and follow [references/supplemental-documents.md](references/supplemental-documents.md); it supports bounded `paste` or Schema 4 `mcp-research`, never GitHub, text-file, or schema-3 `mcp-read`. External evidence and supplements always require exact-package approval and cannot use a standing profile. Treat the path as local CLI input only: write the task, model, app/workspace labels, and approval explanation using the safe LABEL, not the absolute path. Preparation fails if that source path is copied into outbound metadata. Use explicit `mcp-read` only when the user specifically requests the legacy three-tool reader; use `mcp-research` for workspace map, multi-range reads, multi-query search, precomputed diff/evidence, and a read-only Codex context-note ledger. Read [references/web-mcp.md](references/web-mcp.md), [references/standing-approval.md](references/standing-approval.md), and, for research, [references/mcp-research.md](references/mcp-research.md).
...
4. Tell the user the destination, purpose, resolved transport, exact outbound artifacts, included count/bytes, Git SHA, dirty-state summary, every supplemental document label/size/hash, and all security findings. By default ask for exact-package approval and stop. If the repository already has an active standing profile, run its match through `approve --repo <repo> --standing-approval <name>` only after verification; report the matched profile and do not ask again. A mismatch is not permission to widen or silently fall back.
5. For manual approval, run `scripts/gptpro.py approve ... --approved-by user --confirm-transmission`; Schema 3/4 also needs `--confirm-mcp-disclosure`, and Schema 4 needs `--confirm-analysis-ledger`. To create a standing profile, first use one manually approved, directed Schema 4 package, show the complete `standing-approval-create --dry-run` scope, obtain explicit profile-creation approval, then write it with `--confirm-standing-approval`. Follow [references/standing-approval.md](references/standing-approval.md).
6. Read [references/browser-handoff.md](references/browser-handoff.md). For `github`, also read [references/github-transport.md](references/github-transport.md). For Web MCP, follow [references/web-mcp.md](references/web-mcp.md) and the schema-specific reference: create/select the intended Tunnel profile once, use bounded secretless discovery on later tasks, and never recursively scan user directories. A chosen default stores only the exact profile name/hash. Run the capability probe only with a user-reviewed trusted `tunnel-client`, and run the secretless profile check before activation. If only the pinned Python path drifted, show the exact current profile hash and obtain separate approval before atomic profile refresh; never refresh automatically. Raw Tunnel ID input is for attended initialization/refresh or explicit legacy compatibility, not routine restarted-task preparation. Require official `doctor` preflight, explicit app/workspace confirmation, and the activation-owned owner-only Unix health socket. Treat the session as active only after exact-child PID and successful control-plane poll evidence. Keep the controller foregrounded; never switch transport automatically.
...
For user onboarding and plain-language operation, read [references/user-manual.md](references/user-manual.md). For detailed CLI examples and lifecycle rules, read [references/workflow.md](references/workflow.md). Read [references/standing-approval.md](references/standing-approval.md) whenever the user wants fewer repeated approvals or a package claims standing authority. Read [references/failure-reporting.md](references/failure-reporting.md) whenever work fails, blocks, or lacks expected evidence. Read [references/supplemental-documents.md](references/supplemental-documents.md) whenever selected external file content must be included without a browser upload. Read [references/response-monitor.md](references/response-monitor.md) whenever a submitted response may outlive the current turn. For scanner and data-handling policy, read [references/security.md](references/security.md). For artifact fields, read [references/manifest-schema.md](references/manifest-schema.md). Read [references/web-mcp.md](references/web-mcp.md) only for an explicit Web MCP request.
...
Exact-package approval is the default and expires if verification fails or any outbound artifact hash changes. A valid bounded standing profile is the only exception and still creates an exact package receipt after a strict subset check. Use wording equivalent to:
...
An earlier general request to use `$gptpro` is not the action-time approval. Wait for a clear answer after presenting the manifest summary unless an already active standing profile matches the verified package. Never infer or create a standing profile from general consent.
