...
An additive Schema-4 approval may record `approval_source = gptpro-standing-approval-v1`, the profile name/hash/expiry, and a hash of the local repository binding. This does not change the manifest schema or approval basis. The CLI first proves the prepared package is a strict subset of the active repository-local profile, then writes the same exact manifest/file-set/Tunnel-bound approval receipt used by manual approval. Existing receipts without these optional provenance fields remain valid. Standing profiles are local policy files, not package artifacts, and are described in [standing-approval.md](standing-approval.md).
