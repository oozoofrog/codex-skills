...
An attended read-only run can contain identical tool-and-argument requests more than once. Until an outer Tunnel command identifier can be correlated safely, describe this only as **upper-layer duplicate dispatch observed at the connector boundary**, not “Pro fan-out” or an intentional parallel plan. gptpro neither retries nor deduplicates an accepted inbound `tools/call`: every physical attempt consumes call budget, and every successfully committed response contributes its bytes again to the disclosure budget and audit. This conservative accounting remains mandatory; deduplication is deferred until a trustworthy operation identity and idempotency contract exist, and write-capable tools remain out of scope.
