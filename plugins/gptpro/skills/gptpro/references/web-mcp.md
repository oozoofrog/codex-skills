...
Changing any axis or switching between the two Web MCP schemas after approval requires revoking/stopping the current session and preparing a new package. That package needs a new manual approval unless it is Schema 4 and independently matches an active bounded standing profile. Never silently fall back to GitHub, paste, text-file, manual, ZIP upload, or desktop-renderer automation.
...
The generated prompt includes the exact package-specific hard limits and tells the advisory model not to rely on broader static schema defaults. Its first package-info request is pinned to `include_paths=true` and `path_page_size=1`; search requests must explicitly set their result, context, and path bounds. Calls and rejections that reach the governance boundary and are durably audited consume the approved call budget; malformed protocol frames rejected before that boundary do not. This guidance prevents a model from exhausting a narrow session merely while discovering its limits without weakening fail-closed accounting.
...
`mcp-audit.jsonl` is an append-only hash chain. New audit schema 2 binds `disclosure_accounting = complete_model_visible_result_v1`; this counts the canonical UTF-8 bytes of every complete model-visible schema-3/4 success result. Schema versions are exact JSON integers. Actual schema-1 schema-3 logs remain verifiable and closable for compatibility but are explicitly labeled `legacy_tool_body_estimate` and cannot accept new calls; matching absence in package/global accounting bindings is required. Schema-4 always carries the current pair, and removing fields from a schema-2 chain is rejected rather than treated as legacy. The current schema/mode are also bound in machine-global authorization, package session state, and the activation receipt; the activation-time header hash is rechecked before stop/recover changes evidence. The log records approved relative path/range/hash/returned-byte metadata and lifecycle summaries, not full response bodies, raw search queries, credentials, or raw environment data. A content result is released only after its audit event is appended and synced. Audit failure therefore denies content. If an exact append may have committed, the runtime reconciles only verified cumulative counters, latches the process session, and best-effort faults/closes persistent authorization. Do not retry; stop/recover the session. An audit record proves that local disclosure was committed for return, not that the network delivered it or ChatGPT used it.
...
For `tools/call`, the [MCP 2025-11-25 schema](https://github.com/modelcontextprotocol/modelcontextprotocol/blob/main/schema/2025-11-25/schema.json) requires `name` while permitting optional object-valued `arguments` and `_meta`. The adapter accepts only those three keys, normalizes omitted `arguments` to `{}`, discards `_meta` before dispatch, and bounds a structurally valid tool name to 128 UTF-8 bytes. Unknown keys, non-object values, task augmentation, malformed calls, and premature `tools/list` remain protocol-level denials. The request-scoped no-notification compatibility path still opens only for an advertised tool. Once the normal lifecycle is ready, a structurally valid unadvertised name reaches the governance runtime only far enough to commit a zero-content rejection; it can never select or execute a tool. A successful rejection consumes one approved call and returns a JSON-RPC `-32602 Invalid params` error whose data carries stable code `MCP_INVALID_ARGUMENT`, rather than a successful JSON-RPC envelope containing `CallToolResult.isError`. The owner-only audit stores the reserved `<unadvertised>` label and a SHA-256 binding of the requested name rather than the raw name. A pre-append failure leaves the counter available for the next call; an ambiguous post-append failure is fail-closed and must not be automatically retried. Neither compatibility behavior is authorization: every content-capable call still revalidates the exact active package, immutable hashes, expiry, budgets, and disclosure audit before returning content.

An attended read-only run can contain identical tool-and-argument requests more than once. Without the explicit correlation diagnostic, describe a concrete three-copy observation only as **upper-layer 3× duplicate dispatch observed at the connector boundary**, not “Pro fan-out” or an intentional parallel plan. gptpro neither retries nor deduplicates an accepted inbound `tools/call`: every durably audited physical call consumes call budget, while a pre-append failure consumes none; every successfully committed response contributes the canonical bytes of its complete model-visible MCP tool result again to the disclosure budget and audit. This conservative accounting remains mandatory, and write-capable tools remain out of scope.

Before any future write-tool design review, an operator may run a separate approved E2E with `mcp-activate --diagnose-request-correlation` only when `mcp-probe` confirms the exact pinned `0.0.12+881c9a8...` private contract. The diagnostic verifies that the activation-owned Unix socket peer is the exact child, applies one total snapshot deadline after authorization revoke, replaces allowlisted outer request IDs with per-session HMACs, and aligns forwarded-only events by order with the post-stop receipt-bound protocol trace and disclosure audit. Stable internal join hashes—including the requested-name hash that distinguishes separate `<unadvertised>` rejections—are stripped before terminal output. It can distinguish repeated copies of one outer request from distinct outer requests only when every sequence, contract shape, window, count, and audited tool event matches; terminal errors, late responses, or zero-tool windows remain `inconclusive`. It never changes budgets or deduplicates. See [Request-correlation diagnostic](request-correlation.md).
...
The trace never records JSON-RPC IDs, client information/capabilities, arguments, paths, queries, content, Tunnel identity, credentials, environment, or raw unknown method/version strings. Its bounded nonblocking lock and no-follow owner/mode checks fail closed before further protocol dispatch, without modifying the independent disclosure audit. A footer distinguishes ordinary stdio EOF, EOF after an observed parent shutdown, and a broken protocol stream. The exact official Tunnel `0.0.12+881c9a8...` closes the MCP child's stdin and immediately sends it `SIGTERM`; that race previously allowed Python's default termination to bypass the trace `finally` block. During an active authorized runtime only, gptpro now replaces that default with a handler whose sole action is an in-memory flag assignment. It does not write, close a stream, raise asynchronously, or fabricate evidence. `parent_shutdown` is written only if the normal read loop subsequently observes real EOF; an unexpected exception or flush failure remains `protocol_broken`, and a forced stop before finalization remains `closed: false`.

Successful activation receipts bind the header hash; a pre-activation connector failure stores a separate package/receipt header binding so the consumed failed attempt remains inspectable. Before exact-child termination that failure snapshot reports `header_binding_valid: true`, `artifact_identity_bound: false`, and `lifecycle_binding_valid: false`. Once termination is observed, a distinct `mcp_activation_stopped` receipt can bind the exact final failed-trace artifact; if no protocol trace ever existed, only machine-global state records the positive child-stop fact and no package final binding is invented. An active-session `mcp_stopped` receipt separately binds its observed final head/count/truncation/closed status. A forced termination with no footer is recorded honestly as a valid prefix with `closed: false`; a safely readable malformed trace binds its raw SHA-256 and byte count, while an unsafe or unavailable artifact explicitly reports that artifact identity is unbound. None of these conditions erases authorization-denial, disclosure-audit, or exact-child-stop evidence.
...
The command verifies the trace binding/hash chain, cross-checks it against the applicable activation/failure and final stop receipt, and prints the independent disclosure-audit totals beside it. `header_binding_valid` distinguishes the activation header from final artifact/lifecycle binding. `terminal_evidence` separately exposes normal `runtime_stop_observed`, failed-activation `activation_failure_stop_observed`, protocol footer/EOF, and final artifact-to-receipt binding. For either receipt-bound stop type, the final comparison detects a self-consistent post-stop rewrite whose unkeyed chain was recomputed. A recorded invalid artifact remains readable as `artifact_valid: false` with only its stable error code. A handshake-only trace with `tool_calls: 0` and `disclosed_bytes: 0` is diagnostic evidence of protocol traffic, not proof that ChatGPT completed a consultation or received a flushed response.

## Attended lifecycle

1. The user creates the intended Tunnel and initializes one owner-only gptpro profile in an attended first-use step. Tunnel ID and runtime-key references are needed for init/refresh, not routine restarted-task preparation.
2. Run secretless `preflight` for the repository and explicit `mcp-read|mcp-research` transport. It selects an exact current default or the sole ready profile without executing the Tunnel client, resolving credentials, creating a package, or disclosing repository content. Ambiguous profiles require explicit selection; a stale default or changed profile fails closed.
3. Prepare the explicit Web MCP transport with the preflight's exact profile name/hash and the smallest useful directed file selection. Verify the package and inspect the complete maximum-disclosure contract, including the package-bound Tunnel hash and profile source/hash. `auto` remains GitHub-first and never selects MCP.
4. Obtain the two package-specific approval confirmations. A general request to use `$gptpro` is not this approval. Schema-4 `mcp-research` may instead use an already created bounded standing profile only after a strict package match; see [standing-approval.md](standing-approval.md). Selecting a different Tunnel never matches that profile and requires a new manual approval or separately approved profile.
5. The user provides the runtime key only for activation, enables ChatGPT Developer Mode, selects the intended account/workspace/app, and reviews the selected schema's discovered tools. Login, MFA, CAPTCHA, OAuth/key creation, and account/workspace permissions are human steps.
6. Run the secretless probe and official `doctor` preflight. Neither intentionally sends a prompt or repository content from the gptpro wrapper. The first probe may create the canonical owner-only runtime directory (`0700`) and lock file (`0600`) while inspecting the empty authorization slot, and it executes the selected or PATH-discovered `tunnel-client` in bounded version/help subprocesses. Therefore, review and trust that external binary before probing it; an arbitrary executable is outside the wrapper's no-disclosure claim. The probe does not create a ChatGPT conversation or resolve credentials. `parent_shutdown_contract_supported: true` means the exact pinned version may receive the narrow EOF/SIGTERM compatibility behavior; false leaves the child's normal process-signal semantics unchanged.
...
10. Stop promptly. `mcp-stop` denies authorization first, finalizes audit/receipt evidence when possible, then sends a cooperative request to the exact owning controller. It never scans process names, uses broad `pkill`, kills unrelated processes, or deletes the Tunnel/profile/package. `authorization_revoked` is true only for a verified `revoked` package receipt; emergency `faulted` denial reports `authorization_denied: true` without claiming revoke success.

   If package/audit/receipt verification—or the handoff directory itself—is unavailable at stop time, the emergency path instead atomically marks only the exact global handoff/session binding `faulted`, leaves every available package/audit/receipt byte untouched, reports `PACKAGE_EVIDENCE_UNAVAILABLE`, and requests cooperative stop through that session's exact controller socket. This denies further content access but is not proof that the audit or receipt was finalized. A later positive exact-child return code is retained only in machine-global state with the applicable receipt flag false. If the same package is restored and verifies, a matching retry may bind that one receipt without changing the prior child evidence. The socket acknowledgement and controller lease release are both insufficient by themselves; `mcp-stop` checks durable evidence even when the acknowledgement is lost. When a concurrent new activation has already replaced the global active pointer, it accepts only the exact old session/handoff's fully validated owner-only terminal archive as stop evidence.
...
- An activation failure must not leave content authorization active. The controller faults the exact machine-global binding before reading or writing package evidence; damaged package bytes therefore cannot keep authorization `activating`. Before recording package-local failure evidence it rechecks the exact global status while holding the package lock, so a completed external `mcp-stop` revoke wins and a late failure observer cannot rewrite `revoked` evidence as `faulted`. `SIGINT`/`SIGTERM`/`SIGHUP` are blocked before the durable begin and handed to the supervisor until its handlers exist, eliminating the pre-supervisor orphan window. Child creation, cooperative-stop acknowledgement, and final publication share one gate. The factory-adjacent exact-session check holds the global runtime lock through Popen ownership, so a terminal denial committed first creates no child. A parsed pending stop blocks publication, while an isolated same-PID exec launcher restores the original mask in the child. Cleanup keeps those signals blocked through durable denial, exact-child termination, terminal stop evidence, and controller-lease release. The Tunnel enters a separate process session so group delivery cannot terminate it before the controller revokes authorization and signals the exact child. The package lock keeps the durable active commit and its bounded nonblocking local announcement ahead of an external `mcp-stop`; announcement output failure faults the activation instead of blocking indefinitely. Stop only the exact child that this controller started. For a genuine activation failure, after its return code is observed, use the distinct activation-stop evidence path: create `mcp_activation_stopped` only when a package-bound failed trace exists, otherwise retain the positive fact in machine-global state without inventing a receipt. Never create the normal active/revoked `mcp_stopped` receipt or an audit footer for a genuine activation failure.
- A controller crash or lost control socket leaves authorization denied/revoked; recovery never guesses a PID or uses a broad kill. Denial, a terminal status, and lease release do not clear the one-session slot: a new activation requires exact-child stop evidence or a separately recorded attended `--confirm-orphan-tunnel-stopped` assertion after external process inspection. That assertion remains distinct from `tunnel_runtime_stopped` and package receipts.
