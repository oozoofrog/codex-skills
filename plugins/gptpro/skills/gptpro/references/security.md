...
The structure validator parses the packaged Python schema as syntax and inspects its static contract. It never imports or executes the candidate module; an unexpected top-level statement, assignment, call, decorator, tool name, or annotation fails validation instead of becoming validator-side code execution.
