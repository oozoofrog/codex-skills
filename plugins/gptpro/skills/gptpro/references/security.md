...
Selected Git filenames are always literal pathspecs. Repository reads walk each parent directory from the repository descriptor with `O_DIRECTORY | O_NOFOLLOW` and open the final file relative to that descriptor; an ancestor symlink cannot substitute outside content. HEAD-relative deletions remain selectable before and after staging, including deletion-only file lists. They use the same include/exclude and secret-path rules, and their patch content is secret-scanned. Diff generation forces text output even for `-diff` attributes or NUL-containing content, so Git binary-patch encoding cannot conceal removed bytes from strict UTF-8 validation or secret scanning. Deleted paths are recorded in the manifest's `diff.deleted_paths` and the inline diff header's `deleted_paths`, count toward the file limit, and participate in standing approval path checks. Current-version packages without this optional deletion field retain the same inline structure. Cross-version package compatibility is not supported. Verification rejects an actual `GIT binary patch` marker line in the inline Git diff with `DIFF_BINARY_ENCODED`, within current-version packages; they must be prepared again before approval or transmission. The same phrase in ordinary file content or prefixed text-diff lines remains valid.

HEAD deletion context is independent of current filesystem state. Files retained by `git rm --cached` enter current file blocks only with untracked selection enabled and retain `tracked: false`; they cannot inherit a tracked-only standing approval. A current untracked file and a deletion may share a path, counted once in the file limit and exact approval path list. File/directory replacements preserve both selected identities. Diff pathspecs explicitly exclude descendants, processing selected descendants separately, so an exact historical path cannot disclose unselected replacement children. Deleted paths are inspected without reading replacement contents, and symlink traversal is still rejected.

The manifest records `runtime_version`, which must equal `schema.RUNTIME_VERSION` before any package verification, approval, consultation, or response recovery. Missing or different versions fail with `PACKAGE_VERSION_UNSUPPORTED`, including packages approved before an update. Historical files remain untouched and are not migrated or reused. Runtime releases must increment this version and keep CLI and Plugin metadata aligned.
