...
2. Open the actual model submenu. Some DOM snapshots include collapsed inert menu items; verify the submenu is open and the target is not inert before clicking. Select the chosen available model, then inspect that model's effort options. A family/Power/Latest preset is not proof of an exact model. The other surface's menu does not establish availability here.
